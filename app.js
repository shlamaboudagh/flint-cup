...
function addGame() {
  ...
  const parseDate = s => new Date(s.date.replace(/(\d+)(st|nd|rd|th)/, "$1") + " " + year);
  schedules[year][teamA].sort((a,b) => parseDate(a) - parseDate(b));
  schedules[year][teamB].sort((a,b) => parseDate(a) - parseDate(b));
